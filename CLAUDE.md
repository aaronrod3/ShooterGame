...
## MCP / Live Editor Sessions

- Claude connected via UE5 MCP operates on the live editor, not source files — safe to inspect and modify Blueprint graphs (AnimBPs, BP_Rifle, DA_RifleBaseConfig) directly.
- Do not use MCP to modify C++-backed UPROPERTY defaults that are meant to be set in code (check ShooterAnimInstanceBase.h first).
- Save/compile Blueprints explicitly after MCP edits — do not assume autosave.
- Known MCP limitation: some AnimGraph node internals are not exposed via the reflection API — confirmed for BlendListByEnum's per-branch enum bindings and LayeredBoneBlend's branch-filter/bone list (layerSetup). Any node using these will need a manual in-editor step after MCP wires the connectable pins. Flag this possibility upfront when asking Claude to build/modify LayeredBoneBlend or BlendListByEnum nodes.

## Off-Limits / DO NOT

- Content/**/*.uasset — do not read/parse as raw binary via filesystem tools (Rider, grep, cat). Inspecting or editing Blueprint graphs through the UE5 MCP server (live editor API) is allowed and expected for AnimBP/Blueprint work.
